...
export default function ExpertTracker() {
  ...
  // Save
  useEffect(() => { try { localStorage.setItem("expert-v3-done", JSON.stringify(done)); console.log("SAVED:", JSON.stringify(done)); } catch {} }, [done]);
  ...
  const phaseProgress = (pid) => {
    const phaseDays = DAYS.filter(d => d.phase === pid);
  ...
  const streak = (() => {
    let s = 0;
    for (const d of DAYS) { if (dayComplete(d.day)) s++; else break; }
    return s;
  })();
  ...
  return (
    <div style={{ minHeight: "100vh", background: BG, color: TEXT, fontFamily: "'SF Pro Display', 'Noto Sans Thai', -apple-system, system-ui, sans-serif" }}>
      ...
      <div style={{
        ...
        <button onClick={() => { if (confirm("ลบ progress ทั้งหมด?")) { setDone({}); setPretestResults({}); setHintsUsed({}); } }} style={{
          padding: "7px 16px", borderRadius: 99, border: `1px solid ${BORDER}`,
          background: CARD, color: MUTED, fontSize: 11, cursor: "pointer",
        }}>🔄 Reset</button>
      </div>
